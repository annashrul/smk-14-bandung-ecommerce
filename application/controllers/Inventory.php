...
class Inventory extends CI_Controller
{
    ...
    public function adjustment($action=null, $page=1) {
        ...
        if ($action == 'get_data') {
            ...
            $get_produk = $this->m_crud->join_data("produk p", "CONCAT(p.code, ' | ', p.nama, ' | ', ifnull(dp.ukuran, '-'), ' | ', ifnull(dp.warna, '-')) value, dp.id_det_produk, ifnull(dp.ukuran, '-') ukuran, ifnull(dp.warna, '-') warna, p.nama, p.code, ifnull(SUM(ks.stok_in-ks.stok_out), 0) stok, 0 qty, 'tambah' jenis", array("det_produk dp", array("table"=>"kartu_stok ks","type"=>"LEFT")), array("dp.produk=p.id_produk", "ks.det_produk=dp.id_det_produk"), "p.nama like '%".$_POST['query']."%' OR p.code like '%".$_POST['query']."%'", null, "dp.id_det_produk");
            ...
            $det_produk = array();
            foreach ($read_data as $row) {
                $get_produk = $this->m_crud->get_join_data("produk p", "dp.id_det_produk, ifnull(dp.ukuran, '-') ukuran, ifnull(dp.warna, '-') warna, p.nama, p.code, ifnull(SUM(ks.stok_in-ks.stok_out), 0) stok", array("det_produk dp", array("table"=>"kartu_stok ks","type"=>"LEFT")), array("dp.produk=p.id_produk", "ks.det_produk=dp.id_det_produk"), "dp.id_det_produk='".$row['det_produk']."'", null, "dp.id_det_produk");
                if ($row['jenis']=='tambah') {
